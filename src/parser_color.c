...
// 		return(err_fail("Error: missing color identifier in the map\n"));
// 	// if(pars.status == 2)
// 	// 	return(err_fail("Error: wrong color range in the map\n"));
// 	if(pars.status == 3) //used
// 		return(err_fail("Error: color is repited in the map\n"));
// 	if(pars.status == 4)
// 		return(err_fail("Error:[4] something wrong with RGB colors in the map\n"));
// 	if(pars.status == 5)
// 		return(err_fail("Error: RGB color is not digit\n"));
// 	if(pars.status == 6)
// 		return(err_fail("Error: wrong range: RGB color smaller than 0 or bigger than 255\n"));
// 	return (EXIT_SUCCESS);
// }
